...
from application.features.auth.permissions import (
    _expand_roles,
    require_user_access,        # <- any logged-in user
    require_teacher_access,     # <- advisors/teachers
    require_admin_access,       # <- admins only
)
...
from application.features.users.crud.user_invitations import (
    complete_user_invite,
    create_invited_user,
    get_user_id_from_invite_token,
)
...
from application.features.users.crud.user_queries import (
    get_all_users_with_roles_allowed,
...
async def get_users(
    ...
    """
    Retrieves users with their roles. Any authenticated user may call this,
    but results are filtered by the caller's role hierarchy:
    ...
    tutor_user_id = user_data.get("user_id") if "Peer Tutor" in caller_roles else None
    ...
    try:
        ...
        for r in flat:
            ...
            tutored_map[r["tutor_id"]].append(
                {"student_id": r["student_id"], "name": year_name}
            )
    ...
    return [
        UserResponse(
            ...
            email=u["email"],
            school_email=u["gt_email"],
            roles=u.get("roles"),
            role_ids=u.get("role_ids"),
            profile_picture_url=u.get("profile_picture_url"),
            is_active=u.get("is_active", True),
            profile_tag=u.get("profile_tag"),
            invite_url=u.get("invite_url"),
            student_profile=(
                StudentProfile(student_id=u["student_id"], year_name=u["year_name"])
                if "student_id" in u and "year_name" in u
                else None
            ),
            tutored_students=tutored_map.get(u["id"], []),
        )
        for u in users
    ]
...
async def get_profile_picture_defaults():
    defaults = fetch_all("ProfilePictureDefaults")
    return [DefaultProfilePicture(id=p["id"], url=p["profile_picture_url"]) for p in defaults]


@router.get("/{user_id}", response_model=UserResponse)
async def get_user_by_id(user_id: int, user_data: dict = Depends(require_teacher_access)):
    """
    Retrieves a user by ID (teacher/advisor access).
    """
    ...
    return UserResponse(
        ...
        student_profile=(
            StudentProfile(student_id=user["student_id"], year_name=user["year_name"])
            if "student_id" in user and "year_name" in user
            else None
        ),
    )
...
async def invite_user(
    ...
    admin_data: dict = Depends(require_teacher_access), 
):
    """
    Invite a new user to complete account setup.
    """
    ...
    result = create_invited_user(
        email, request_data.school_email, request_data.role_ids, request_data.student_type
    )
...
async def delete_user(user_id: int, user_data: dict = Depends(require_admin_access)):
    """
